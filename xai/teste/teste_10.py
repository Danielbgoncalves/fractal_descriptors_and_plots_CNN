...
for nome_dataset, classes, test_dir, predicoes_csv, num_classes, output_dir, models_dir in [
    ("displasia", ["healthy", "severe"], displasia_test_dir,
     r"resultados_displasia\predicoes.csv", 2, r"resultados_displasia\occlusion", r"resultados_displasia\models"),

    ("pulmao", ["aca_md", "nor", "scc_md"], lung_test_dir,
     r"resultados_pulmao\predicoes.csv", 3, r"resultados_pulmao\occlusion", r"resultados_pulmao\models"),
]:
    ...
    df_resultado, df_resumo = analisar_limiar_de_decisao(
        ...
        margem_max=0.1, n_confiantes=15,models_dir=models_dir, metodo="occlusion"
    )
    print(df_resumo)

    fig = gerar_figura_exemplos(test_dataset, df_resultado, "mobilenet_recplot", seed=7, metodo="occlusion", models_dir=models_dir, num_classes=num_classes)
