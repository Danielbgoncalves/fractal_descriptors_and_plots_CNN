...
def pixel_to_descriptoor(
        row: int, col: int, 
        n_descritores: int = TAMANHO_ORIGINAL_DESCRITORES,
        tamanho_final=TAMANHO_REDE
        ):
    '''
    mapeia um pixel (row, col) do heatmap para o par de descritore originais
    (descritor_row, descritor_col) no espaço 180x180
    '''

    fator = n_descritores / tamanho_final

    desc_row = int(row * fator)
    desc_col = int(col * fator)

    desc_row = min(max(0, desc_row), n_descritores - 1)
    desc_col = min(max(0, desc_col), n_descritores - 1)

    info_row = decodifica_indice_descrtor(desc_row)
    info_col = decodifica_indice_descrtor(desc_col)

    return (desc_row, desc_col), (info_row, info_col)

def decodifica_indice_descrtor(idx: int):
    '''
    Dado indice de 0 a 179 retorna a métrica, a função (p, g ou h) e o raio correspondente
    '''
...
def definir_bandas_metricas():
    '''
    Retorna as ttres faixas do recplot pelas 3 MÉTRICAS de distância
    '''
    return {
        "Minkowski": (0, 60),
        "Euclidiana": (60, 120),
...
def definir_bandas_funcoes():
    '''
    Retorna as 9 sub-faixas em MÉTRICAS + FUNÇÃO (p, g, h)
    '''
...
def overlay_bandas_recplot(
        heatmap: np.ndarray,
        img_rec_np: np.ndarray,
        detalhar_funcoes: bool=False,
        alpha_heatmap: float=0.5
        ):
    '''
    Desenha o RecPlot com o heatmap de atribuição e as grades exatas das métricas
    '''
    ...
    ax.set_title("XAI RecPlot - Mapeamento por Métricas de Distancia e Percolação", fontsize=10)
    ax.axis("off")
    ax.tight_layout()

    return fig

def avaliar_importancia_por_metricas_e_funcao(
    ...
    for nome_banda, (inicio, fim) in subbandas.tems():
        ...
        with torch.no_grad():
            out_oclusao = torch.softmax(modelo(img_oclusa), dim=1)
            prob_oclusa = out_oclusao[0, target_class].item()
