...
def run_kfold(dataset_path, dataset_type, class_names, backbone="mobilenet", seed=SEED, output_dir="resultados"):
    ...
    # USADO APENAS NO DATASET DA DISPLASIA            
    groups = []
    for p in paths:
        filename = os.path.basename(p)
        # print(f"Processando arquivo: {filename}")
        parts = filename.split('.')[0].split('-')
        # print(f"Partes do nome do arquivo: {parts}")
        patient_id = f"{parts[0]}-{parts[1]}"
        groups.append(patient_id)

    if dataset_type == "RP_perc":
        current_transform = transform_recplot # transform_recplot
        # Para RecPlots, inicialização aleatória (treina do zero) é o ideal
        peso_carregado ='DEFAULT' # None 
    else:
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(paths, labels, groups)):
        print("\n============================")
        ...
        train_dataset = ImageDataset(train_data, transform=current_transform)
        val_dataset   = ImageDataset(val_data, transform=current_transform)
